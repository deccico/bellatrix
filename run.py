...
class Run():
    def __init__(self, key, sec, app_name, burn_at_the_end=False): #todo move all the globals here
    ...
    def run(self):
        ...
        for config in configs:
            ...
            self._ec2.waitForConnectionReady(inst, user, PK, dns_name, TIME_OUT=300)
            r = self.executeCommands(user, inst.dns_name, PK, commands)
            if self.burn_at_the_end:
                new_ami = self._ec2.createImage(inst.id, config + "-" + datetime.datetime.now().isoformat(), "generated by " + self._app_name)
                logging.info("ami: %s is being generated for configuration: %s" % (new_ami, config))
